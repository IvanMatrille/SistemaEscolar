...
namespace WindowsForm32018.Registros
{
    public partial class frmConsultaAsignatura : Form
    {
        ...
        DataTable dt;
        Session session;
        public frmConsultaAsignatura(Session session)
        {
            InitializeComponent();
        ...
        string buildFiltro()
        {
            ...
            {
                filtro.AppendFormat("AND (Descripcion+' '+Observaciones+' '+Clave) LIKE '%{0}%' ",
                                this.txtNombre.Text);
            }
        ...
        private void btnModificar_Click(object sender, EventArgs e)
        {
            ...
            if (id > 0)
            {
                frmAsignaturas frm = new frmAsignaturas(id, session);
                frm.StartPosition = FormStartPosition.CenterScreen;
                if (frm.ShowDialog() == DialogResult.OK)
                {
                    btnBuscar_Click(sender, e);
